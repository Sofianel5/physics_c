...
function Fix(s) {
    sEnd = s.lastIndexOf("&nbsp;")
    if (sEnd == s.length - 6) { s = s.substring(0, sEnd) }
    s = s.replace(/&nbsp;/g, " ")
    s = s.replace(/&#8203;/g, "‍")
    ...
    setTimeout(function() {
        if (document.getElementsByClassName('_2ck9StMwG9')[0]) {
            ...
            if (Question !== LastQuestion) {
                ...
                if (QuizData.medias) { Questions = QuizData.medias[0].questions } else { Questions = QuizData.questions }
                        ...
                        if (Questions[i].choices) {
                            ...
                            for (let x = 0; x < Questions[i].choices.length; x++) {
                                if (Questions[i].choices[x].isCorrect) {
                                    ...
                                    for (let y = 0; y < Choices.length; y++) { if (Fix(Choices[y].children[1].children[0].children[0].innerHTML) == Answer) { Choices[y].children[1].children[0].children[0].innerHTML = "<correct-answer-x3Ca8B><u>" + Choices[y].children[1].children[0].children[0].innerHTML + "</u></correct-answer-x3Ca8B>" } }
                                }
                            }
